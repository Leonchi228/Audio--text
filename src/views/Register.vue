...
const handleRegister = async () => {
    ...
    if (error) {
        ...
        toast.info('Данный адрес электронной почты уже зарегистрирован. Пожалуйста, войдите в систему.')
    } else {
        toast.success('Вы успешно зарегистрировались! Пожалуйста, подтвердите свою электронную почту.')
        router.push('/login')
    }
}
</script>
...
<template>
    <div class="min-h-screen w-full flex items-center justify-center bg-slate-50 p-4 sm:p-8 font-sans">
        ...
        <div class="bg-white max-w-md w-full px-6 py-8 sm:px-10 sm:py-10 rounded-2xl border border-slate-100 shadow-xl text-center">
            ...
            <form @submit.prevent="handleRegister" class="space-y-5 sm:space-y-6 text-left">
