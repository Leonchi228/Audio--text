...
if (error) {
    ...
    toast.success('Письмо для восстановления пароля успешно отправлено! Проверьте свою почту.')
}}
...
<template>
    ...
    <div v-if="!isEmailSent" class="bg-white max-w-md w-full px-10 py-10 rounded-2xl border border-slate-100 shadow-xl text-center h-11/10">
        ...
        <div v-if="isEmailSent" class="bg-white max-w-md w-full px-10 py-22 rounded-2xl border border-slate-100 shadow-xl text-center h-11/10">
            <h2 class="text-3xl font-bold mb-5 text-slate-900 tracking-tight">
                Письмо отправлено
            </h2>
            <p class="text-slate-500 mb-5">Проверьте свою почту и следуйте инструкциям для восстановления пароля.</p>
            <router-link to="/login" class="font-medium text-slate-900 hover:text-blue-600 transition-colors ml-1.5">
                Вернуться к входу
            </router-link>
        </div>
